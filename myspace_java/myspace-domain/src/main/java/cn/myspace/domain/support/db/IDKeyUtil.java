package cn.myspace.domain.support.db;
 ...
 * 41位时间截(毫秒级)
 *      注意：41位时间截不是存储当前时间的时间截，而是存储时间截的差值（即 当前时间截 - 开始时间截 而得到的值)
 *           这里的的开始时间截，一般是id生成器开始使用的时间，由程序来指定的
 *      41位的时间截可以使用 T = (1L << 41) / (1000L * 60 * 60 * 24 * 365) = 69年 
 * 10位的数据机器位
 *      可以部署在1024个节点，包括5位机房id(datacenterId)和5位机器id(workerId)
 * 12位序列
